...
<!--= = = ATTRIBUTE OUTPUT RULES  = = = = = = = = =-->
   ...
   <xsl:template match="*" mode="audience.att.out">
      ...
      <xsl:attribute name="audience">
         <!-- START Oxygen Patch (EXM-49692) -->
         <xsl:apply-templates select="." mode="translate.profiling.values.separator">
            <xsl:with-param name="value" select="$value"/>
         </xsl:apply-templates>
         <!-- END Oxygen Patch (EXM-49692) -->
      </xsl:attribute>
   </xsl:template>
   ...
   <!-- START Oxygen Patch (EXM-49692) -->
   <xsl:template match="*" mode="translate.profiling.values.separator">
      <xsl:param name="value"/>
